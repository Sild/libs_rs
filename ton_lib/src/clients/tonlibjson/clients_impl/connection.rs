...
use crate::clients::tonlibjson::client_raw::tl_types::{TLBlockId, TLConfig, TLOptions, TLOptionsInfo};
...
impl TLJClient for TLJConnection {
    async fn get_connection(&self) -> Result<&dyn TLJClient, TonlibError> { Ok(self) }
...
fn run_loop(tag: String, weak_inner: Weak<Inner>) {
    ...
    loop {
        let inner = match weak_inner.upgrade() {
            Some(inner) => inner,
            None => {
                // callback.on_connection_loop_exit(&tag);
                break;
            }
        };
        let (response, maybe_extra) = match inner.client_raw.receive(1.0) {
            Err(err) => {
                // callback.on_error(&tag, err);
                continue;
            }
            Ok(None) => {
