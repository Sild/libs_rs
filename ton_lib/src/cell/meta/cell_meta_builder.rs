...
impl<'a, T: TonCell> CellMetaBuilder<'a, T> {
    ...
    pub(super) fn validate(&self) -> TonLibResult<()> {
    ...
    fn validate_ordinary(&self) -> TonLibResult<()> {
        if self.data_bits_len as u32 > CellMeta::CELL_MAX_DATA_BITS_LEN {
            return Err(TonLibError::CellBuilderMeta("Ordinary cell data bits length is too big".to_owned()));
        }
        Ok(())
    }

    fn validate_pruned(&self) -> TonLibResult<()> {
        if self.refs_count > 0 {
            return Err(TonLibError::CellBuilderMeta("Pruned cell can't have refs".to_owned()));
        }
        if self.data_bits_len < 16 {
            return Err(TonLibError::CellBuilderMeta("PrunedBranch require at least 16 bits data".to_owned()));
        }
        ...
        if level_mask == LevelMask::MIN_LEVEL || level_mask > LevelMask::MAX_LEVEL {
            ...
            return Err(TonLibError::CellBuilderMeta(err_msg));
        }
    ...
    fn validate_library(&self) -> TonLibResult<()> {
    ...
    fn validate_merkle_proof(&self) -> TonLibResult<()> {
        // type + hash + depth
        ...
        if self.refs[0].is_none() || self.refs[1].is_some() {
            return Err(TonLibError::CellBuilderMeta("Merkle Proof cell must have exactly 1 ref".to_owned()));
        }

        let mut data_slice = &self.data[1..];
        let _proof_hash = TonHash::try_from(&data_slice[..TonHash::BYTES_LEN])
            .map_err(|err| TonLibError::CellBuilderMeta(format!("Can't get proof hash bytes from cell data: {err}")))?;
    ...
    fn validate_merkle_update(&self) -> TonLibResult<()> {
    ...
    pub(super) fn calc_hashes_and_depths(&self, level_mask: LevelMask) -> TonLibResult<([TonHash; 4], [u16; 4])> {
    ...
    fn get_repr_for_data(
        ...
        let result = writer
            .writer()
            .ok_or_else(|| TonLibError::CellBuilderMeta("Stream for cell repr is not byte-aligned".to_string()))?
            .to_vec();
    ...
    fn write_ref_hashes(&self, writer: &mut CellBitWriter, level: u8) -> TonLibResult<()> {
    ...
    fn write_ref_depths(&self, writer: &mut CellBitWriter, level: u8) -> TonLibResult<()> {
        for ref_pos in 0..self.refs_count {
    ...
    fn calc_pruned_hash_depth(&self, level_mask: LevelMask) -> TonLibResult<Vec<Pruned>> {
...
fn write_data(writer: &mut CellBitWriter, data: &[u8], bit_len: usize) -> TonLibResult<()> {
