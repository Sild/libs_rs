...
use ton_lib::clients::tonlibjson::clients_impl::TLJClientDefault;
...
pub(crate) async fn make_tlj_client_default(mainnet: bool, archive_only: bool) -> anyhow::Result<TLJClientDefault> {
    init_logging();
    log::info!("initializing tlj_client with mainnet={mainnet}...");
    let net_conf = get_net_conf(mainnet)?;
    let config = TLJClientConfig::new(net_conf, archive_only);
    Ok(TLJClientDefault::new(config).await?)
}
