use std::collections::HashMap;
use crate::errors::TLBResult;
use crate::primitives::numbers::TLBNumber;
use crate::primitives::reference::Ref;
use crate::tlb_type::TLBType;
use ton_lib_cell::build_parse::builder::TonCellBuilder;
use ton_lib_cell::build_parse::parser::TonCellParser;
use ton_lib_cell::cell::ton_cell::ArcTonCell;
use ton_lib_cell::cell::ton_hash::TonHash;
use crate::primitives::tlb_object::TLBObject;

// https://github.com/ton-blockchain/ton/blob/59a8cf0ae5c3062d14ec4c89a04fee80b5fd05c1/crypto/block/block.tlb#L281
#[derive(Debug, Clone, PartialEq)]
pub struct StateInit {
    pub split_depth: Option<TLBNumber<u8, 5>>,
    pub tick_tock: Option<TickTock>,
    pub code: Option<Ref<ArcTonCell>>,
    pub data: Option<Ref<ArcTonCell>>,
    // there is likely library:(HashmapE 256 SimpleLib)
    // pub library: Option<Ref<TLBObject<HashMap<TonHash, ArcTonCell>>>>,
    pub library: Option<Ref<ArcTonCell>>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct TickTock {
    pub tick: bool,
    pub tock: bool,
}

impl StateInit {
    pub const fn new(code: ArcTonCell, data: ArcTonCell) -> Self {
        StateInit {
            split_depth: None,
            tick_tock: None,
            code: Some(Ref::new(code)),
            data: Some(Ref::new(data)),
            library: None,
        }
    }
}

impl TLBType for StateInit {
    fn read_def(parser: &mut TonCellParser) -> TLBResult<Self> {
        Ok(StateInit {
            split_depth: TLBType::read(parser)?,
            tick_tock: TLBType::read(parser)?,
            code: TLBType::read(parser)?,
            data: TLBType::read(parser)?,
            library: TLBType::read(parser)?,
        })
    }

    fn write_def(&self, dst: &mut TonCellBuilder) -> TLBResult<()> {
        self.split_depth.write(dst)?;
        self.tick_tock.write(dst)?;
        self.code.write(dst)?;
        self.data.write(dst)?;
        self.library.write(dst)?;
        Ok(())
    }
}

impl TLBType for TickTock {
    fn read_def(parser: &mut TonCellParser) -> TLBResult<Self> {
        let tick = parser.read_bit()?;
        let tock = parser.read_bit()?;
        Ok(TickTock { tick, tock })
    }

    fn write_def(&self, builder: &mut TonCellBuilder) -> TLBResult<()> {
        builder.write_bit(self.tick)?;
        builder.write_bit(self.tock)?;
        Ok(())
    }
}

#[cfg(test)]
mod tests {
    // use std::ops::Deref;
    //
    // use super::*;
    // use crate::cell::{BagOfCells, TonCellError};
    //
    // #[test]
    // fn test_state_init_regular_contract() -> Result<(), TonCellError> {
    //     // state_init of UQCJ7Quj9gM_SE3uwOk3gEJC2JFQcgg0s7CSpLr7B_2yiHPG contract
    //     let state_init_hex = "";
    //     let source_cell = BagOfCells::parse_hex(state_init_hex)?.single_root()?;
    //     let state_init = source_cell.parser().load_tlb::<StateInit>()?;
    //     assert_eq!(state_init.split_depth, None);
    //     assert_eq!(state_init.tick_tock, None);
    //     assert!(state_init.code.is_some());
    //     assert!(state_init.data.is_some());
    //     assert_eq!(state_init.library, None);
    //     let serial_cell = CellBuilder::new().store_tlb(&state_init)?.build()?;
    //     assert_eq!(source_cell.deref(), &serial_cell);
    //
    //     let parsed_back = StateInit::from_cell(&serial_cell)?;
    //     assert_eq!(state_init, parsed_back);
    //     Ok(())
    // }
}
